...
router.post('/', (req, res) => {
    ...
    db.query('INSERT INTO buku (nama_buku) VALUES (?)', [nama_buku.trim()], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
        const newBook = { id: results.insertId, nama_buku: nama_buku.trim() };
...
router.put('/:id', (req, res) => {
    ...
    db.query('UPDATE buku SET nama_buku = ? WHERE id = ?', [nama_buku.trim(), req.params.id], (err, results) => {
        if (err) return res.status(500).send('Internal Server Error');
        if (results.affectedRows === 0) return res.status(404).send('Buku tidak ditemukan');
        res.json({ id: parseInt(req.params.id), nama_buku: nama_buku.trim() });
    });
});
